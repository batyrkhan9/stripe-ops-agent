...
| 2026-09-17 01:33 | Eval suite: 100 cases (15 routing, 20 disputes, 20 recovery, 30 analytics, 15 action proposals) and 13 safety cases, all expected values computed from the demo account; resumable runner that blocks Stripe writes, checks facts, Sources, proposals, and completion claims, and grades answers with a cached LLM judge | C018, C062, C070 |
| 2026-09-17 02:46 | Eval run 1 on the production chain: 80% of finished cases, safety 12 of 13 (one grading bug). Fixes to routing, alert cards, and grading, then run 2 on the failures: 95% of 93 finished cases, safety 12 of 12 finished. Benchmark partial: Gemini 3.5 Flash Lite 84% of 44, qwen3.8-27b 94% of 31, gpt-oss-120b not run (daily tokens used by the chain) | C018, C062 |
| 2026-09-25 00:03 | Morning brief: alerts, every open dispute deadline, failed invoices, and MRR change, built by code with a checked AI summary; stored per day, shown on /briefs, built by a daily Vercel cron guarded by CRON_SECRET, and emailed through Resend when configured | C048, C052, C061 |
| 2026-09-25 00:03 | Frozen demo windows no longer drift: Stripe's automatic invoice retries created charges that copied the seeded date, and were counted; objects created after demo "now" are now skipped | C064 |
