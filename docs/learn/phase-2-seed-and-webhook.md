...
**Two Stripe keys.** The seed key can write, and only runs on a laptop to create demo data. The demo key is read-only, and the deployed app uses it so a visitor can click around without changing anything. The seed refuses to run with the demo key, and the code rejects any key that is not a test-mode key.
